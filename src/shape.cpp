    ...
    -> Shape::ObjectIdSet
{
    ...
    auto vboId = makeVertexBufferObject(vertices);

    auto eboId = makeVertexIndexBufferObject(indices);
...
Shape::Shape(std::vector<float> const & vertices, std::vector<unsigned int> const & indices)
    : objectIds{makeShape(vertices, indices)}
...
auto makeSquare()
    -> Shape
{
    auto const vertices = std::vector<float>{
    ...
    auto indices = std::vector<unsigned int>(vertices.size(), 0);
